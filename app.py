...
if not st.session_state.bulut_user:
    ...
    if st.sidebar.button("Bulut'a Bağlan ⚡"): st.session_state.bulut_user = user.strip().lower(); st.rerun()
else:
...
with sag:
    ...
    if st.button("Sorgula 🚀") and u_input:
        ...
        try:
            ...
            if user not in st.session_state.gecmis: st.session_state.gecmis[user] = []
            st.session_state.gecmis[user].append((u_input, res.text))
            st.rerun()
